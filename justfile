...
# Architecture coverage: every top-level src/agnova/ component must have a
...
ci-arch:
    ...
    pkg_dir = pathlib.Path("src/agnova")
    ...
    if not components:
        print("Architecture coverage: no top-level components under src/agnova/ yet — nothing to check.")
        sys.exit(0)

    if not arch_doc.exists():
        print(f"::error::ARCHITECTURE.md is missing but src/agnova/ has {len(components)} component(s): {', '.join(components)}", file=sys.stderr)
        sys.exit(1)
