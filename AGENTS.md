...
Run an AI agent as a first-class participant in a Buzz workspace.

## Project structure

```
ARCHITECTURE.md                  — component docs; just ci-arch enforces coverage (see below)
src/agnova/   — application source (see import contracts in .importlinter)
tests/                           — pytest suite
...
Every top-level module or package directly under `src/agnova/` needs a
